...
describe('Top-level Ibm', () => {
  ...
  public class Response {
    ...
    test('produces buildable proj | without explicit Req/Res', () => {
      ...
      path.join(os.tmpdir(), runuuid, 'ibm', 'build.gradle')

      const fn = jest.fn(() => execSync(`gradle build -b ${path.join(os.tmpdir(), runuuid, 'ibm', 'build.gradle')}`))
      fn()
